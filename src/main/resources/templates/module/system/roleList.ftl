    ...
    <div class="layui-col-xs12">
        <script type="text/html" id="toolbar">
            <div class="layui-btn-container">
                <@sa.hasPermission name="${PrivCode.BTN_CODE_ROLE_SAVE}">
                    <button class="layui-btn layui-btn-sm" lay-event="add"><i class="layui-icon layui-icon-addition"></i>添加</button>
                </@sa.hasPermission>
                <@sa.hasPermission name="${PrivCode.BTN_CODE_ROLE_DELETE}">
                    <button class="layui-btn layui-btn-sm layui-btn-danger" lay-event="delete"><i class="layui-icon layui-icon-delete"></i>删除</button>
                </@sa.hasPermission>
            </div>
        </script>
        <script type="text/html" id="operator">
            <@sa.hasPermission name="${PrivCode.BTN_CODE_ROLE_ALLOT}">
